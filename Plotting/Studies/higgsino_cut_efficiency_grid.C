...
void higgsino_cut_efficiency_grid() {
    ...
    TCut additional_cut("Ptll>40");

    bool make_grid_plot = true;
    ...
    if (make_grid_plot) {
        ...
        canvas->Print("Plots/higgsino_efficiency_grid.eps");
    }
    ...
    if (make_table) {
        string filePath = "/eos/atlas/atlascerngroupdisk/phys-susy/2L2J-ANA-SUSY-2018-05/SusySkim2LJets/v1.7/SUSY2/";
        vector<vector<float>> efficiencies;
        vector<string> periods{"mc16a", "mc16cd", "mc16e"};
        for (auto period : periods) {
            cout << "." << flush;
            string ttbar_filename = filePath + "SUSY2_Bkgs_" + period + "/ttbar_merged_processed.root";
            string Zjets_filename = filePath + "SUSY2_Bkgs_" + period + "/Zjets_merged_processed.root";
            string Wjets_filename = filePath + "SUSY2_Bkgs_" + period + "/Wjets_merged_processed.root";
            string diboson_filename = filePath + "SUSY2_Bkgs_" + period + "/diboson_merged_processed.root";

            TChain* tch_ttbar = new TChain(); tch_ttbar->Add((ttbar_filename + "/ttbar_NoSys").c_str());
            TChain* tch_bkg = new TChain(); tch_bkg->Add((ttbar_filename + "/ttbar_NoSys").c_str());
            tch_bkg->Add((Zjets_filename + "/Zjets_NoSys").c_str());
            tch_bkg->Add((Wjets_filename + "/Wjets_NoSys").c_str());
            tch_bkg->Add((diboson_filename + "/diboson_NoSys").c_str());

            cout << "." << flush;
            tch_ttbar->Draw(">> baseline_cut_event_list_ttbar", baseline_cut);
            cout << "." << flush;
            tch_ttbar->Draw(">> additional_cut_event_list_ttbar", baseline_cut + additional_cut);
            cout << "." << flush;
            tch_bkg->Draw(">> baseline_cut_event_list_bkg", baseline_cut);
            cout << "." << flush;
            tch_bkg->Draw(">> additional_cut_event_list_bkg", baseline_cut + additional_cut);
            cout << "." << flush;
            TEventList *baseline_cut_event_list_ttbar = (TEventList*)gDirectory->Get("baseline_cut_event_list_ttbar");
            TEventList *baseline_cut_event_list_bkg = (TEventList*)gDirectory->Get("baseline_cut_event_list_bkg");
            TEventList *additional_cut_event_list_ttbar = (TEventList*)gDirectory->Get("additional_cut_event_list_ttbar");
            TEventList *additional_cut_event_list_bkg = (TEventList*)gDirectory->Get("additional_cut_event_list_bkg");
            int n_baseline_cut_passed_ttbar = baseline_cut_event_list_ttbar->GetN();
            int n_baseline_cut_passed_bkg = baseline_cut_event_list_bkg->GetN();
            int n_additional_cut_passed_ttbar = additional_cut_event_list_ttbar->GetN();
            int n_additional_cut_passed_bkg = additional_cut_event_list_bkg->GetN();
            float ttbar_efficiency = float(n_additional_cut_passed_ttbar) / n_baseline_cut_passed_ttbar;
            float bkg_efficiency = float(n_additional_cut_passed_bkg) / n_baseline_cut_passed_bkg;

            vector<float> period_efficiencies;
            period_efficiencies.push_back(ttbar_efficiency);
            period_efficiencies.push_back(bkg_efficiency);
            efficiencies.push_back(period_efficiencies);
            cout << "." << endl;
        }
        ...
        for (int selection=0; selection<2; selection++) {
            string selection_name = "ttbar";
            if (selection == 1)
                selection_name = "bkg";
            for (int period=0; period<periods.size(); period++)
                cout << "& " << efficiencies[period][selection];
            cout << " \\\\" << endl;
        }
