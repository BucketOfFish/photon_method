...
void make_plots(string process, vector<string> trees, vector<string> regions) {
    ...
    string save_folder = "InclusiveMC/";
    ...
    for (auto region : regions) {
        for (auto var : vars) {
            string title = var[2] + " for " + process + " in " + region;
            ...
            can->Print((save_folder + var[3] + "_" + process + "_" + region + ".png").c_str());
            delete hist;
        }
    }
}
...
void make_2D_eta_phi_plot() {
    ...
    vector<string> all_mc_processes = {"diboson", "higgs", "lowMassDY", "singleTop", "topOther", "triboson", "ttbar",
        "Vgamma", "Wjets", "Zjets"};
    //vector<string> processes = all_mc_processes
    vector<string> processes = {"all MC"};
    ...
    vector<string> regions = {"SRC", "SRLow", "SRMed", "SRHigh",
        "VRC", "VRLow", "VRMed", "VRHigh",
        "CRC", "CRLow", "CRMed", "CRHigh"};
    for (auto process : processes) {
        vector<string> trees = {};
        if (process == "all MC") {
            for (auto process : all_mc_processes) {
                vector<string> new_trees = {
